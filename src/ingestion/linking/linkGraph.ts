...
export function getLinkStatsSummary(stats: LinkStats): string {
  const robotPct =
    stats.totalRobots > 0 ? Math.round((stats.linkedRobots / stats.totalRobots) * 100) : 0

  const toolPct =
    stats.totalTools > 0 ? Math.round((stats.linkedTools / stats.totalTools) * 100) : 0

  return (
    `Linked ${stats.linkedRobots}/${stats.totalRobots} robots (${robotPct}%), ` +
