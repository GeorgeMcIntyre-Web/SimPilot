...
export function buildCrossRefInputFromApplyResult(
  ...
  const existingRobots = existingState.assets.filter(
    (a) => a.kind === 'ROBOT',
  ) as unknown as Robot[]
  ...
  const panelMilestonesMap =
    vacuumRows && vacuumRows.length > 0
  ...
  const compareStringsIgnoreCase = (a: string, b: string): number => {
    const aUpper = a.toUpperCase()
    const bUpper = b.toUpperCase()
    const upperCompare = compareStrings(aUpper, bUpper)
    if (upperCompare !== 0) return upperCompare
    return compareStrings(a, b)
  }

  const panelMilestonesKeyByUpper = new Map<string, string>()
  for (const key of panelMilestonesMap.keys()) {
    panelMilestonesKeyByUpper.set(key.toUpperCase(), key)
  }

  const getStationKeyCandidates = (cell: Cell): string[] => {
    ...
    const consider = (value: string | null | undefined) => {
      ...
      const normalized = normalizeStationId(trimmed)
      if (!normalized) return
      const normalizedUpper = normalized.toUpperCase()
      if (seen.has(normalizedUpper)) return
      seen.add(normalizedUpper)
      candidates.push(normalized)
    }

    consider(cell.code)
    consider(cell.stationId ?? undefined)
  ...
  const coerceMetricsToNumbers = (
    raw: Record<string, unknown>,
  ): Record<string, number | undefined> => {
    const result: Record<string, number | undefined> = {}
    const keys = Object.keys(raw).sort(compareStringsIgnoreCase)
    for (const key of keys) {
      const parsed = parseMetricNumber(raw[key])
  ...
  const simulationStatusRows: SimulationStatusSnapshot[] = allCells.map((cell) => {
    const stationKeyCandidates = getStationKeyCandidates(cell)

    // Try to find panel milestones for this cell using multiple key formats
    let panelMilestones: PanelMilestones | undefined
    for (const candidate of stationKeyCandidates) {
      const existingKey = panelMilestonesKeyByUpper.get(candidate.toUpperCase())
      if (!existingKey) continue
      const panels = panelMilestonesMap.get(existingKey)
      if (!panels) continue
      panelMilestones = panels
      break
    }
    ...
    if (panelMilestonesMap.size > 0) {
      const stationUpperSet = new Set(stationKeyCandidates.map((k) => k.toUpperCase()))
      const byRobotUpper = new Map<string, { caption: string; panels: PanelMilestones }>()

      for (const [key, panels] of panelMilestonesMap) {
        const parts = key.split('::', 2)
        if (parts.length !== 2) continue

        const stationPart = parts[0]?.trim()
        const robotPart = parts[1]?.trim()
        if (!stationPart || !robotPart) continue
        if (!stationUpperSet.has(stationPart.toUpperCase())) continue

        const captionUpper = robotPart.toUpperCase()
        if (byRobotUpper.has(captionUpper)) continue
        byRobotUpper.set(captionUpper, { caption: robotPart, panels })
      }

      if (byRobotUpper.size > 0) {
        robotPanelMilestones = {}
        const ordered = Array.from(byRobotUpper.values()).sort((a, b) =>
          compareStringsIgnoreCase(a.caption, b.caption),
        )
        for (const entry of ordered) {
          robotPanelMilestones[entry.caption] = entry.panels
        }
      }
    }
    ...
    if (robotPanelMilestones) {
      ...
      overallCompletion =
        robotCompletions.length > 0
          ? Math.round(robotCompletions.reduce((s, v) => s + v, 0) / robotCompletions.length)
          : undefined
    }

    if (robotPanelMilestones === undefined && panelMilestones) {
    ...
    const pct = (group?: { completion: number; milestones?: Record<string, any> }) => {
      ...
      const values = group.milestones
        ? (Object.values(group.milestones).filter((v) => typeof v === 'number') as number[])
        : []
    ...
    readinessMetrics['1st STAGE SIM COMPLETION'] =
      overallCompletion ?? cell.simulation?.percentComplete
    ...
    const mergedMetricsRaw: Record<string, unknown> = {
      ...(cell.simulation?.metrics ?? {}),
      ...readinessMetrics,
    }
    const numericMetrics = coerceMetricsToNumbers(mergedMetricsRaw)
    const metrics = Object.keys(numericMetrics).length > 0 ? numericMetrics : undefined
  ...
  if (panelMilestonesMap.size > 0) {
    const withMilestones = simulationStatusRows.filter(
      (r) => r.panelMilestones !== undefined,
    ).length
  ...
  if (simulationRobots && simulationRobots.length > 0) {
    ...
    const existingCombinations = new Set(
      robotSpecsRows.map(
        (r) => `${r.stationKey.toUpperCase()}::${(r.caption || '').toUpperCase()}`,
      ),
    )
