...
} from './fixtures/realWorldMock'
import { coreStore } from '../../domain/coreStore'
import { clearCrossRefData } from '../../hooks/useCrossRefData'
import { clearFileTrackingHistory } from '../fileTracker'
...
describe('ingestionCoordinator - Header Sniffing', () => {
  ...
  describe('Multi-File Ingestion', () => {
    it('should merge tools from multiple equipment files', async () => {
      ...
      const file1 = await createFileFromWorkbook(
        createWorkbookFromArray(gunSheet1, 'GunData1'),
        'guns1.xlsx',
      )
      const file2 = await createFileFromWorkbook(
        createWorkbookFromArray(gunSheet2, 'GunData2'),
        'guns2.xlsx',
      )
    ...
    it('should merge data from multiple simulation files', async () => {
      ...
      const file1 = await createFileFromWorkbook(
        createWorkbookFromArray(MESSY_SIMULATION_SHEET, 'SIMULATION'),
        'sim1.xlsx',
      )
      const file2 = await createFileFromWorkbook(
        createWorkbookFromArray(MESSY_SIMULATION_SHEET, 'SIMULATION'),
