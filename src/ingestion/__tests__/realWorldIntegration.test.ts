...
describe('Real-World Integration: Station 010 / Robot R01 Linking', () => {
    ...
    it('should link Station 010 / Robot R01 from Simulation Status to Tool List', async () => {
        ...
        const toolFile = await createFileFromWorkbook(
            toolWorkbook,
        ...
        if (result.warnings.length > 0) {
            console.log('Warnings:', result.warnings.map(w => `${w.kind}: ${w.message}`))
        }
        ...
        const station010Cell = state.cells.find(c =>
            c.code === '010' && c.name.includes('R01')
        )
        expect(station010Cell).toBeDefined()
        expect(station010Cell?.code).toBe('010')

        // Assert: Verify Station 010 tool exists
        const station010Tool = state.assets.find(a =>
            a.stationNumber === '010' && a.name === 'TOOL-001'
        )
    ...
    it('should normalize station codes for linking ("010" → "10")', async () => {
        ...
        const toolFile = await createFileFromWorkbook(
            createWorkbookFromArray(toolData),
            'tools.xlsx'
        )
...
describe('Real-World Integration: Reference Data Population', () => {
    ...
    it('should populate coreStore.referenceData.employees from EmployeeList sheet', async () => {
        ...
        const werner = state.referenceData.employees.find(e =>
            e.name === 'Werner Hamel'
        )
        expect(werner).toBeDefined()
        expect(werner?.id).toBe('EMP-001')

        const lisa = state.referenceData.employees.find(e =>
            e.name === 'Lisa Mueller'
        )
    ...
    it('should populate coreStore.referenceData.suppliers from SupplierList sheet', async () => {
        ...
        const acme = state.referenceData.suppliers.find(s =>
            s.name === 'ACME Robotics GmbH'
        )
    ...
    it('should handle workbook with BOTH EmployeeList and SupplierList sheets', async () => {
        ...
        XLSX.utils.book_append_sheet(
            workbook,
            XLSX.utils.aoa_to_sheet(EXACT_TOOL_LIST),
            'ToolList'
        )
        XLSX.utils.book_append_sheet(
            workbook,
