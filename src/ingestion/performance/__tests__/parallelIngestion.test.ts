 ...
 */
async function createTestFile(workbook: XLSX.WorkBook, fileName: string): Promise<File> {
  ...
  describe('concurrency', () => {
    it('should respect concurrency limit', async () => {
      const files = await Promise.all(
        Array(6)
          .fill(null)
          .map((_, i) => createSimpleFile(`file${i}.xlsx`)),
      )
...
describe('parseFilesWithCallback', () => {
  ...
  it('should call callback for each file', async () => {
    ...
    await parseFilesWithCallback(files, (result, index) => {
...
describe('preloadFilesToCache', () => {
  ...
  it('should preload files into cache', async () => {
    ...
    const files = await Promise.all([
      createSimpleFile('preload1.xlsx', [
        ['H1', 'H2'],
        ['Preload', 1],
      ]),
      createSimpleFile('preload2.xlsx', [
        ['H1', 'H2'],
        ['Preload', 2],
      ]),
    ])
