...
function DetailItem({ label, value, className }: DetailItemProps) {
  return (
    <div className={cn('py-1', className)}>
      ...
      <dd className="mt-0.5 text-xs text-gray-900 dark:text-gray-100">{value || '—'}</dd>
    </div>
  )
}
...
export function AssetDetailPanel({
  ...
  const hasReuseInfo =
    asset.sourcing === 'REUSE' && (oldProject !== undefined || targetProject !== undefined)
  const hasOldLocation =
    oldProject !== undefined || oldLine !== undefined || oldStation !== undefined
  const hasTargetLocation =
    targetProject !== undefined || targetLine !== undefined || targetStation !== undefined
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            ...
            <section className="rounded-lg border border-gray-100 dark:border-gray-800 p-2 bg-gray-50/60 dark:bg-gray-800/40">
              ...
              <dl className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                ...
                {robotNumber !== undefined && <DetailItem label="Robot #" value={robotNumber} />}
                {gunId !== undefined && <DetailItem label="Gun ID" value={gunId} />}
              </dl>
            </section>
          </div>
                ...
                {hasOldLocation && (
                  <div className="rounded-lg border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900/50 p-3">
                    ...
                    <dl className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                      ...
                      {oldArea !== undefined && <DetailItem label="Area" value={oldArea} />}
                      <DetailItem
                        label="Line / Station"
                        value={formatLocation(oldLine, oldStation)}
                ...
                {hasTargetLocation && (
                  <div className="rounded-lg border border-blue-100 dark:border-blue-800 bg-blue-50/60 dark:bg-blue-900/20 p-3">
                    ...
                    <dl className="grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                      ...
                      <DetailItem
                        label="Line / Station"
                        value={formatLocation(targetLine, targetStation)}
                        className="col-span-2"
                      />
                    </dl>
                  </div>
