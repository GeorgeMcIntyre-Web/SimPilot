...
export function createAssetsTableColumns(
  ...
  return [
    ...
    },
    {
      ...
      accessor: (asset) => {
        const station = asset.stationNumber || (asset.metadata?.stationCode as string) || ''
        const area =
          asset.areaName ||
        ...
        // If we found a cell, link to it using its canonical ID
        if (cell) {
          const linkId = (cell as any).stationId || cell.id
          return (
            <Link
              to={`/cells/${encodeURIComponent(linkId)}`}
              className="text-blue-600 dark:text-blue-400 hover:underline"
              onClick={(e) => e.stopPropagation()}
            >
        ...
        // Fallback: Link to the station anyway, assuming the ID might match the code/name
        // This addresses "I need the entries in the station column to be links" even if we can't resolve the cell object
        const linkTarget = normalizeStationCode(station) || station
        return (
          <Link
            to={`/cells/${encodeURIComponent(linkTarget)}`}
            className="text-blue-600 dark:text-blue-400 hover:underline"
            onClick={(e) => e.stopPropagation()}
            title="Station linking inferred"
          >
            {station}
          </Link>
        )
