    ...
    <div className="space-y-4">
      <div>
        <h3 className="typography-title-sm text-gray-900 dark:text-gray-100">
          Diff Results (live from store)
        </h3>
        <p className="typography-subtitle text-gray-500 dark:text-gray-400">
          These rows are built directly from `coreStore.diffResults`, so they stay in sync with
          ambiguity resolution and reactivations.
        </p>
      </div>
          ...
          <table className="w-full table-auto divide-y divide-gray-300 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-800">
              <tr>
                <th
                  scope="col"
                  className="py-3.5 pl-4 pr-3 text-left typography-label text-gray-900 dark:text-gray-100 sm:pl-6"
                >
                  Import
                </th>
                <th
                  scope="col"
                  className="px-3 py-3.5 text-left typography-label text-gray-900 dark:text-gray-100"
                >
                  Filename
                </th>
                <th
                ...
                <th
                  scope="col"
                  className="px-3 py-3.5 text-center typography-label text-gray-900 dark:text-gray-100"
                >
                  Created
                </th>
                <th
                ...
                return (
                  <Fragment key={entry.importRunId}>
                    <tr
                      ...
                      onClick={() => toggleRow(entry.importRunId)}
                    >
                      <td className="py-4 pl-4 pr-3 typography-body-strong text-gray-900 dark:text-gray-100 sm:pl-6">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="min-w-0">
                            <div className="typography-body-strong break-words">
                              {entry.importRunId}
                            </div>
                          </div>
                        </div>
                      </td>
                      <td className="px-3 py-4 typography-caption text-gray-700 dark:text-gray-300 break-words">
                        {entry.sourceFile}
                      </td>
                      <td className="px-3 py-4 typography-caption text-gray-700 dark:text-gray-300 break-words">
                        {entry.plantKey}
                      ...
                      <td className="px-3 py-4 typography-caption text-center text-gray-700 dark:text-gray-300">
                        {summary.created}
                      </td>
                      <td className="px-3 py-4 typography-caption text-center text-gray-700 dark:text-gray-300">
                        {summary.updated}
                      ...
                      <td className="px-3 py-4 typography-caption text-gray-700 dark:text-gray-300">
                        ...
                        <td colSpan={10} className="bg-gray-50 dark:bg-gray-800/60 px-6 py-4">
                          <div className="space-y-4">
                            ...
                            <DiffSection
                              title="Deleted"
                              items={entry.deletes}
                              renderItem={renderDelete}
                            />
                            <DiffSection
                              title="Renamed / Moved"
                              items={entry.renamesOrMoves}
                              renderItem={renderRename}
                            />
                            <DiffSection
                              title="Ambiguous"
                              items={entry.ambiguous}
                              renderItem={renderAmbiguous}
                            />
                          </div>
                        </td>
                      </tr>
    ...
    <div>
      ...
      <div className="mt-2 space-y-2">
        {items.map((item, idx) => (
          <div
            key={idx}
            className="rounded-md border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 p-3 typography-body text-gray-900 dark:text-gray-100"
          >
            {renderItem(item)}
          </div>
        ))}
      ...
      <ul className="text-sm list-disc list-inside space-y-1 text-gray-900 dark:text-gray-100">
        {item.candidates.map((candidate) => (
          <li key={candidate.uid}>
            <span className="font-medium">{candidate.key}</span> — score {candidate.matchScore}% (
