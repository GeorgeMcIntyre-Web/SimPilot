...
import { StationRecord } from '../../domain/uidTypes'

type StationRegistryTableProps = {
  stations: StationRecord[]
  searchTerm: string
  onToggleActive: (station: StationRecord, reason?: string | null) => void
  onAddAlias: (station: StationRecord, alias: string, reason?: string) => void
}

export function StationRegistryTable({
  stations,
  searchTerm,
  onToggleActive,
  onAddAlias,
}: StationRegistryTableProps) {
  ...
  const handleToggleClick = (station: StationRecord) => {
    const reason = prompt(
    ...
    onToggleActive(station, reason)
  }

  const handleAddAliasClick = (station: StationRecord) => {
    if (!aliasInput.trim()) return
    onAddAlias(station, aliasInput.trim(), reasonInput.trim() || undefined)
    setAliasInput('')
    setReasonInput('')
    setEditingUid(null)
  }
            ...
            <tr key={station.uid} className={station.status === 'inactive' ? 'opacity-50' : ''}>
              ...
              <td className="px-4 py-3 whitespace-nowrap text-sm">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handleToggleClick(station)}
                  ...
                  {editingUid === station.uid ? (
                    <div className="flex flex-col gap-1">
                      ...
                      <div className="flex gap-1">
                        <button
                          onClick={() => handleAddAliasClick(station)}
