...
function asReuseStatus(status?: string | null): ReuseAllocationStatus | null {
  ...
  const allowed: ReuseAllocationStatus[] = [
    'AVAILABLE',
    'ALLOCATED',
    'IN_USE',
    'RESERVED',
...
export function ToolsPage() {
  ...
  const columns: Column<Tool>[] = [
    ...
    },
    {
      header: 'Subtype',
      accessor: (t) =>
        t.subType ? <Tag label={humanizeSubType(t.subType) ?? t.subType} color="gray" /> : '-',
    ...
    },
    {
      header: 'Area',
      accessor: (t) => {
        const area = t.areaName || (t as any).areaGroup || '—'
        if (t.projectId) {
          return (
            <Link
    ...
    },
    {
      ...
      accessor: (t) =>
        t.sourceFile ? (
          <span className="text-xs text-gray-500" title={t.sourceFile}>
  ...
  const appliedFilterChips = [
    typeFilter !== 'ALL'
      ? {
          label: `Type: ${humanizeToolType(typeFilter as ToolType)}`,
          onClear: () => setTypeFilter('ALL'),
        }
      : null,
    subTypeFilter !== 'ALL'
      ? {
          label: `Subtype: ${humanizeSubType(subTypeFilter as SpotWeldSubType)}`,
          onClear: () => setSubTypeFilter('ALL'),
        }
      : null,
    mountFilter !== 'ALL'
      ? { label: `Mount: ${humanizeMountType(mountFilter)}`, onClear: () => setMountFilter('ALL') }
      : null,
    reuseFilter !== 'ALL'
      ? { label: `Reuse: ${reuseFilter}`, onClear: () => setReuseFilter('ALL') }
      : null,
    lineFilter !== 'ALL'
      ? { label: `Line: ${lineFilter}`, onClear: () => setLineFilter('ALL') }
      : null,
    areaFilter !== 'ALL'
      ? { label: `Area: ${areaFilter}`, onClear: () => setAreaFilter('ALL') }
      : null,
    stationFilter !== 'ALL'
      ? { label: `Station: ${stationFilter}`, onClear: () => setStationFilter('ALL') }
      : null,
    sourceFilter !== 'ALL'
      ? { label: `Source: ${sourceFilter}`, onClear: () => setSourceFilter('ALL') }
      : null,
    searchTerm.trim() !== ''
      ? { label: `Search: “${searchTerm}”`, onClear: () => setSearchTerm('') }
      : null,
        ...
        {filteredTools.length === 0 ? (
          ...
          <DataTable
            ...
            keyExtractor={(t) =>
              t.id ??
              t.canonicalKey ??
              `${t.toolType}-${t.name}-${t.stationCode ?? ''}-${t.oemModel ?? ''}`
            }
            onRowDoubleClick={(t) => {
              if (t.id) {
