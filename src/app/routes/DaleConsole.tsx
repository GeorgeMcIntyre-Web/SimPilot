...
export function DaleConsole() {
  ...
  const [hasSeenIntro, setHasSeenIntro] = useState(() =>
    getUserPreference('simpilot.dale.hasSeenIntro', false),
  )

  if (metrics.totalCells === 0) {
    return (
      <div
        data-testid="dale-console"
        className="space-y-8 pb-12 bg-[radial-gradient(circle_at_top,_#ffe4e6_0,_#ffffff_45%)] -m-6 p-6 min-h-screen"
      >
  ...
  const atRiskCells = cells
    .filter((c: Cell) => {
      if (!c.simulation) return false
      const isAtRisk =
        c.simulation.hasIssues ||
        (c.simulation.percentComplete > 0 &&
          c.simulation.percentComplete < 100 &&
          c.status === 'Blocked')
  ...
  const atRiskColumns: Column<Cell>[] = [
    ...
    { header: 'Engineer', accessor: (c) => c.assignedEngineer || '-' },
    {
      header: 'Issues',
      accessor: (c) =>
        c.simulation?.hasIssues ? <span className="text-red-600 font-bold">Yes</span> : 'No',
    },
  ]
  ...
  const engineerColumns: Column<(typeof engineerMetrics)[0]>[] = [
    ...
    { header: 'Cells', accessor: (e) => e.cellCount },
    {
      header: 'At Risk',
      accessor: (e) =>
        e.atRiskCellsCount > 0 ? (
          <span className="text-red-600 font-bold">{e.atRiskCellsCount}</span>
        ) : (
          '0'
        ),
    },
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <KpiTile
            ...
            progress={
              metrics.atRiskCellsCount > 0
                ? Math.min(100, (metrics.atRiskCellsCount / metrics.totalCells) * 100)
                : 0
            }
            status={
              metrics.atRiskCellsCount === 0
                ? 'success'
                : metrics.atRiskCellsCount > 5
                  ? 'danger'
                  : 'warning'
            }
          ...
          <KpiTile
            ...
            progress={
              lateCellsCount > 0 ? Math.min(100, (lateCellsCount / metrics.totalCells) * 100) : 0
            }
          ...
          <KpiTile
            ...
            progress={
              scheduleAtRiskCount > 0
                ? Math.min(100, (scheduleAtRiskCount / metrics.totalCells) * 100)
                : 0
            }
            status={
              scheduleAtRiskCount === 0 ? 'success' : scheduleAtRiskCount > 3 ? 'danger' : 'warning'
            }
          />
          {changes.length > 0 && (
            <KpiTile
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <section className="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-4 flex items-center">
            ...
            Engineers Summary{' '}
            {zenMode && <span className="ml-2 text-xs text-gray-500">(At Risk Only)</span>}
          </h3>
          <DataTable
            data={zenMode ? engineerMetrics.filter((e) => e.atRiskCellsCount > 0) : engineerMetrics}
            columns={engineerColumns}
            emptyMessage={
              zenMode ? 'No engineers with at-risk cells.' : 'No engineer data available.'
            }
          />
        </section>
      </div>
            ...
            {themeMode === 'flower' && (
              ...
              >
                {chaosCopied ? (
                  ...
                  <FlowerAccent className="h-4 w-4 mr-2 text-rose-500" />
                )}
