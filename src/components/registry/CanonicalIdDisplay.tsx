 ...
 */
export function CanonicalIdDisplay({
  plantKey,
  uid,
  entityKey,
  className = '',
}: CanonicalIdDisplayProps) {
  const [copied, setCopied] = useState(false)

  // Extract short UID (last 8 chars of UUID part)
  const uidShort = uid.split('_')[1]?.slice(-8) || uid.slice(-8)
  const canonicalId = `${plantKey}-${uidShort}-${entityKey}`
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <span className="font-mono text-sm text-gray-900 dark:text-gray-100">{canonicalId}</span>
      <button
        onClick={handleCopy}
