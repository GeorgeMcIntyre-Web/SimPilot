...
export function FlagBadge({ flag, compact = false, className }: FlagBadgeProps) {
  ...
  return (
    <div className={cn('flex items-start gap-3 p-3 rounded-lg border', style.container, className)}>
      <div className={cn('flex-shrink-0 mt-0.5', style.icon)}>
        <SeverityIcon severity={flag.severity} />
      </div>
      <div className="flex-1 min-w-0">
        <p className={cn('text-sm font-medium', style.text)}>{label}</p>
        <p className={cn('text-xs mt-0.5', style.subtext)}>{flag.message}</p>
        {flag.stationKey && (
...
const severityChip = (severity: 'ERROR' | 'WARNING') => {
  ...
  if (severity === 'ERROR')
    return (
      <span className={`${base} bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-200`}>
        Error
      </span>
    )
  return (
    <span
      className={`${base} bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-200`}
    >
      Warning
    </span>
  )
...
function FlagsTable({ flags }: FlagsTableProps) {
  ...
  const columns: Column<FlagsTableRow>[] = [
    ...
    {
      header: 'Message',
      accessor: (r) => <span className="text-gray-600 dark:text-gray-300">{r.message}</span>,
    },
