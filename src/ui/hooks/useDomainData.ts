...
export function useGlobalSimulationMetrics() {
  ...
  const avgCompletion =
    cellsWithSimulation.length > 0
      ? Math.round(
          cellsWithSimulation.reduce((acc, c) => acc + (c.simulation?.percentComplete || 0), 0) /
            cellsWithSimulation.length,
        )
      : 0
...
export function useAllProjectMetrics() {
  ...
  return projects.map((p) => {
    ...
    const cellsWithSim = projectCells.filter(
      (c) => c.simulation && c.simulation.percentComplete > 0,
    )
    const avgCompletion =
      cellsWithSim.length > 0
        ? Math.round(
            cellsWithSim.reduce((acc, c) => acc + (c.simulation?.percentComplete || 0), 0) /
              cellsWithSim.length,
          )
        : 0
...
export function useAllEngineerMetrics() {
  ...
  return Object.entries(engineers)
    .map(([name, engineerCells]) => {
      const cellsWithSim = engineerCells.filter(
        (c) => c.simulation && c.simulation.percentComplete > 0,
      )
      const avgCompletion =
        cellsWithSim.length > 0
